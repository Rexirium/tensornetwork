...
let 
    file1 = h5open("KitaevChain/kcbenchdata2.h5", "r")
    muls = read(file1, "muls")
    runtime = read(file1, "runtime")
    usespace = read(file1, "usespace")
    close(file1)

    file2 = h5open("KitaevChain/kcplotdata2.h5", "r")
    mus = read(file2, "mus")
    ...
    b1 = bar(mucoord, runtime, xlabel=L"\mu/t", ylabel= "runtime (s)", yscale=:log10, ylim=(0.1,100), 
        bar_width = 0.1, width=0.5, label=alglabel, legend_position=:topleft,leftmargin=2Plots.mm)
    ...
    savefig(b1, "KitaevChain/kitaevfigs/kc_bench1.svg")
    ...
    savefig(b2, "KitaevChain/kitaevfigs/kc_bench2.svg")
